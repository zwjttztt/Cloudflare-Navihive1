...
- 🔑 **站点账号密码** — 每个站点卡片可保存登录账号/密码，支持一键复制与密码显隐
- 💾 **备份与恢复** — 顶部「备份」按钮支持备份到本地文件 / WebDAV，并可从本地或远端备份恢复
...
1. 鼠标悬停任意站点卡片 → 点击右上角齿轮图标打开「网站设置」。
2. 在「登录凭据」区域填写**账号**与**密码**，点击输入框右侧的复制图标即可一键复制（密码框左侧的眼睛图标可切换明文显示）。
3. 点击「保存」后凭据会存入 D1 数据库，并随备份文件一起导出/恢复。

### 备份与恢复（顶部「备份」按钮）

点击页面顶部的 **备份** 按钮打开「数据备份与恢复」窗口：
...
**如何备份数据？** 登录后点击顶部「备份」按钮，可备份到本地或 WebDAV；或命令行 `wrangler d1 database export navigation-db`。
